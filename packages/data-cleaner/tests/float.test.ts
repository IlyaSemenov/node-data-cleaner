...
t.test("reject string", async (t) => {
	t.rejects(clean.float()("test"), new ValidationError("Invalid value."))
})

t.test("reject boolean", async (t) => {
	t.rejects(clean.float()(true), new ValidationError("Invalid value."))
})

t.test("reject object", async (t) => {
	t.rejects(clean.float()({}), new ValidationError("Invalid value."))
})
...
t.test("Empty values", async (t) => {
	await t.test("reject undefined", async (t) => {
		t.rejects(clean.float()(), new ValidationError("Value required."))
	})
	...
	await t.test("reject null", async (t) => {
		t.rejects(clean.float()(null), new ValidationError("Value required."))
	})
