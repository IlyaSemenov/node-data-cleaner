...
describe('Any', function() {
	...
	it('should not allow default if required set to true', function() {
		expect(() => clean.string({ default: 123, required: true })).to.throw(
			SchemaError,
		)
		expect(() => clean.string({ default: 123, required: false })).to.not.throw(
			SchemaError,
		)
		expect(() => clean.string({ default: null, null: false })).to.throw(
			SchemaError,
		)
		expect(() => clean.string({ default: null, null: true })).to.not.throw(
			SchemaError,
		)
...
describe('String', function() {
	...
	describe('Empty values', function() {
		...
		it('should not allow blank to null conversion if blank set to false', function() {
			expect(() => clean.string({ blank: null, null: false })).to.throw(
				SchemaError,
			)
			expect(() => clean.string({ blank: null, null: true })).to.not.throw(
				SchemaError,
			)
...
describe('Date', function() {
	...
	describe('Empty values', function() {
		...
		it('should not allow blank to null conversion if blank set to false', function() {
			expect(() => clean.date({ blank: null, null: false })).to.throw(
				SchemaError,
			)
			expect(() => clean.date({ blank: null, null: true })).to.not.throw(
				SchemaError,
			)
...
describe('Integer', function() {
	...
	it('should cast string if cast allowed', async function() {
		await clean
			.integer({ cast: true })('123.45')
			.should.become(123)
	})
	it('should not allow bad string', async function() {
		await clean
			.integer({ cast: true })('bummer')
			.should.be.rejectedWith(ValidationError)
	})
...
describe('Float', function() {
	...
	it('should cast string if cast allowed', async function() {
		await clean
			.float({ cast: true })('123.45')
			.should.become(123.45)
	})
	it('should not allow bad string', async function() {
		await clean
			.float({ cast: true })('bummer')
			.should.be.rejectedWith(ValidationError)
	})
	it('should not allow empty string', async function() {
		await clean
	...
	it('should allow empty string if null is allowed', async function() {
		await clean
			.float({ cast: true, null: true })('')
			.should.become(null)
	})
	it('should allow empty string if default is null', async function() {
		await clean
			.float({ cast: true, default: null })('')
			.should.become(null)
	})
	it('should not allow empty string if null is allowed but cast is disabled', async function() {
		await clean
...
describe('Object', function() {
	...
	describe('Multiple fields', function() {
		...
		it('should reject if some fields not present', async function() {
			await cleaner({ s2: 'two' }).should.be.rejectedWith(
				ValidationError,
				'{"s1":["Value required."],"s3":["Value required."]}',
			)
		})
	})
	...
	it('should store custom field cleaner validation error in proper field key', async function() {
		await clean
			...
			.should.be.rejectedWith(ValidationError, '{"obj":["bang"]}')
	})
	it('should store custom field cleaner field-aware validation error in proper field key', async function() {
		await clean
			...
			.should.be.rejectedWith(ValidationError, '{"obj.foo":["bang"]}')
	})
	it('should store nested object custom field cleaner validation error in proper field key', async function() {
		await clean
			...
			.should.be.rejectedWith(ValidationError, '{"obj1.obj2":["bang"]}')
	})
	it('should store nested object custom field cleaner validation error in proper field key', async function() {
		await clean
			.object({
			...
			.should.be.rejectedWith(ValidationError, '{"obj1.obj2.foo":["bang"]}')
	})
	...
	it('should handle schema.nonFieldErrorsKey in nested field', async function() {
		await clean
			...
			.should.be.rejectedWith(ValidationError, '{"obj.other1":["bang"]}')
	})
...
describe('Array', function() {
	...
	it('should pass array of integers', async function() {
		await clean
			.array({ element: clean.integer() })([1, 2, 3])
			.should.become([1, 2, 3])
	})
	...
	it('should allow string in array of integers if cast is allowed', async function() {
		await clean
			.array({ element: clean.integer({ cast: true }) })([1, '2', 3])
			.should.become([1, 2, 3])
	})
	describe('Min/max', function() {
	...
	it('should collect validation errors from all elements', async function() {
		await clean
			...
			.should.be.rejectedWith(
				ValidationError,
				'["Invalid value: one","Invalid value: three"]',
			)
	})
	it('should collect named validation errors from all elements', async function() {
		await clean
			...
			.should.be.rejectedWith(
				ValidationError,
				'["password: Password is too short.","user: Value required.","user: Value required.","password: Password is too short."]',
			)
	})
	describe('Empty values', function() {
		it('should not accept undefined', async function() {
...
describe('ValidationError', function() {
	...
	it('should accept a string', function() {
		expect(new ValidationError('bang')).to.satisfy(err => {
			return (
				expect(err.messages).to.deep.equal(['bang']) &&
	...
	it('should accept an array', function() {
		expect(new ValidationError(['bang', 'boom'])).to.satisfy(err => {
			return (
				expect(err.messages).to.deep.equal(['bang', 'boom']) &&
				expect(err.errors).to.be.undefined
			)
		})
	})
	...
	it('should not accept an array of junk', function() {
		expect(() => {
			new ValidationError(['bang', null])
		}).to.throw()
	})
	it('should accept an object', function() {
		expect(
			new ValidationError({ s1: 'bang', s2: ['boom', 'oops'] }),
		).to.satisfy(err => {
			return (
				expect(err.errors).to.deep.equal({
					s1: ['bang'],
					s2: ['boom', 'oops'],
				}) && expect(err.messages).to.be.undefined
			)
		})
	})
	...
	it('should not accept an object of junk', function() {
		expect(() => {
			new ValidationError({ s1: 'bang', s2: null })
		}).to.throw()
		expect(() => {
			new ValidationError({ s1: 'bang', s2: [null] })
		}).to.throw()
